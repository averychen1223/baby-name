...
def make_gui(top, width, height, names, draw_names, search_names):
    ...
    entry.bind(
        "<Return>",
        lambda event: handle_draw(entry, canvas, names, error_out, draw_names),
    )
    search_entry.bind(
        "<Return>",
        lambda event: handle_search(search_entry, search_out, names, search_names),
    )
...
def handle_draw(entry, canvas, names, error_out, draw):
    ...
    text = entry.get()
    lookups = [
        name[0].upper() + name[1:].lower() for name in text.split()
    ]  # handles casing
